...
The ARM64 build has one entry point for both local development and CI:
...
- Native CPython 3.14 ARM64 for the WinRT Bluetooth beacon
...
# Remove only ARM64 build outputs
.\build.ps1 clean
```
...
```text
out/arm64/build       CMake and Ninja build
out/arm64/beacon      PyInstaller build
out/arm64/artifacts   MSI and portable ZIP
release               verified runtime staging folder
Bonjour SDK           generated/cached Bonjour ARM64 SDK
```

## How runtime bundling works

`packaging/gstreamer-features-arm64.txt` contains stable GStreamer element names,
...
the active MSYS2 CLANGARM64 environment. ABI filename changes such as
...
```text
packaging/gstreamer-features-arm64.txt
```
