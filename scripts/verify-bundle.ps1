<# Verifies that the ARM64 bundle is complete and independent of the development setup.
...
param(
    ...
    [string]$EnvironmentName = "clangarm64",
...
if (-not $TestCacheDir) {
    $TestCacheDir = Join-Path $projectRoot "out\arm64\test-cache"
}
...
try {
    ...
    $registry = Join-Path $TestCacheDir "registry-arm64.bin"
