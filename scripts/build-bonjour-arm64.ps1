<# Prepares the ARM64 Bonjour SDK required by uxplay-windows.
...
$required = @(
    ...
    (Join-Path $sdk "Bin\arm64\dnssd.dll"),
    (Join-Path $sdk "Bin\arm64\mDNSResponder.exe"),
...
if ($cacheValid) {
    try {
        $buildInfo = Get-Content `
            ...
            -Raw |
            ConvertFrom-Json
        $cacheValid = $buildInfo.architecture -eq "arm64"
    }
    catch {
        $cacheValid = $false
    }
...
if (-not $Force -and $cacheValid) {
    Write-Host "Bonjour SDK ARM64 is already available."
    return
}
...
$sourceDir = Join-Path $ProjectRoot "out\arm64\bonjour-source"
if (Test-Path -LiteralPath $sourceDir) {
    $resolvedSource = [IO.Path]::GetFullPath($sourceDir)
    $expectedRoot = [IO.Path]::GetFullPath((Join-Path $ProjectRoot "out\arm64"))
...
& $msbuild `
    ...
    /m /t:Build /p:Configuration=Release /p:Platform=ARM64
if ($LASTEXITCODE -ne 0) {
...
New-Item -ItemType Directory -Force -Path (Join-Path $sdk "Lib\arm64") | Out-Null
# The current libuxplay CMake file still looks under Lib\x64 on Windows.
# Keep an ARM64 import-library alias there until upstream makes it configurable.
New-Item -ItemType Directory -Force -Path (Join-Path $sdk "Lib\x64") | Out-Null
New-Item -ItemType Directory -Force -Path (Join-Path $sdk "Bin\arm64") | Out-Null
...
Copy-Item `
    ...
    -Force
Copy-Item `
    (Join-Path $sourceDir "mDNSWindows\DLL\arm64\Release\dnssd.lib") `
    (Join-Path $sdk "Lib\arm64\dnssd.lib") `
    -Force
Copy-Item `
    (Join-Path $sourceDir "mDNSWindows\DLL\arm64\Release\dnssd.lib") `
    ...
    -Force
Copy-Item `
    (Join-Path $sourceDir "mDNSWindows\DLL\arm64\Release\dnssd.dll") `
    (Join-Path $sdk "Bin\arm64\dnssd.dll") `
    -Force
Copy-Item `
    (Join-Path $sourceDir "mDNSWindows\SystemService\arm64\Release\mDNSResponder.exe") `
    (Join-Path $sdk "Bin\arm64\mDNSResponder.exe") `
    -Force
...
Write-Host "Bonjour SDK ARM64 built successfully."
